package {
	...
	public class Game extends MovieClip {
		...
		var arrayOfCoolThings: Array = new Array();
		...
		private function updateThings(): void {
			...
			for (var i = arrayOfCoolThings.length - 1; i >= 0; i--) {
				arrayOfCoolThings[i].update();
				if (arrayOfCoolThings[i].isDead) {
					arrayOfCoolThings.splice(i, 1);
				}
		...
		private function spawn(): void {
			...
			if (counter >= 10) {
				...
				arrayOfCoolThings.push(ct);
				trace("added"); // TODO: add the CoolThing object to the array
				addChild(ct);
				counter = 0;
			}
		}
